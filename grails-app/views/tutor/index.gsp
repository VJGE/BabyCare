		...
		<div id="list-tutor" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="registroEPS" title="${message(code: 'tutor.registroEPS.label', default: 'Registro EPS')}" />
					
						<th><g:message code="tutor.hijos.label" default="Hijos" /></th>
				...
				<g:each in="${tutorInstanceList}" status="i" var="tutorInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${tutorInstance.id}">${fieldValue(bean: tutorInstance, field: "registroEPS")}</g:link></td>
					
						<td>${fieldValue(bean: tutorInstance, field: "hijos")}</td>
